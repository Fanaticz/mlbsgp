...
def _extract_player_name(market_name, market_type, subcat_name):
    ...
    for suffix in [market_type, subcat_name]:
        ...
        for pat in [suffix, suffix.replace(" O/U", ""), suffix.replace(" Milestones", ""),
                    suffix.replace("Alternate ", "")]:
            if pat and name.lower().endswith(pat.lower()):
                name = name[:-len(pat)].strip()
                break
    # Fallback: split on known stat keywords (most specific first so "Outs Recorded"
    # catches before "Outs", and "Earned Runs" catches before "Runs").
    if name == market_name:
        for kw in ["Outs Recorded", "Pitching Outs", "Earned Runs Allowed",
                    "Earned Runs", "Hits Allowed", "Strikeouts", "Walks",
                    "Total Bases", "Home Runs", "RBIs", "Stolen Bases",
                    "Singles", "Doubles", "Triples", "Hits", "Runs"]:
...
def find_sgps(legs):
    ...
    for pitcher, plegs in by_pitcher.items():
        ...
        # Debug: when nothing matched, collect DK pitcher-prop market names seen
        # for this pitcher so the UI can show what DK actually offered. Keep the
        # list small to avoid blowing up the response.
        dk_pitcher_markets = []
        if not matched:
            seen = set()
            for p in md["props"]:
                ...
                if not _pitcher_matches(pitcher, p.get("player", "")):
                    continue
                label = (p.get("marketName") or p.get("subcategory") or "").strip()
                if label and label not in seen:
                    seen.add(label)
                    dk_pitcher_markets.append(label)
                if len(dk_pitcher_markets) >= 10:
                    break
        ...
        base = {
            ...
            "dk_pitcher_markets": dk_pitcher_markets,
        }
