...
LIVE_KEYS = ("eventStatus", "eventScorecard", "liveInfo")


def subscribe_msg(query, req_id=1):
    return json.dumps({
        "jsonrpc": "2.0", "id": req_id, "method": "subscribe",
        "params": {"entity": "events",
                   "queryParams": {"query": query, "initialData": True,
                                   "projection": "sportsbook", "locale": "en-US"}},
    })
...
async def run(query, raw, duration):
    ...
    while end is None or time.time() < end:
        try:
            async with websockets.connect(URL, open_timeout=15, additional_headers=HEADERS,
                ...
                await ws.send(subscribe_msg(query))
                backoff = 2
                while end is None or time.time() < end:
...
def main():
    ...
    try:
        asyncio.run(run(query, a.raw, a.seconds))
